...
public class Arduino : MonoBehaviour {
    ...
    // Servo degrees
    [Range(0, 180)]
    public int[] degs = new int[4];
    int[] lastDegs = new int[4];
	const int degSize = 4;
	...
	void Start () {
		...
		arduinoSerial.ReadBufferSize = 8192*128;
		arduinoSerial.WriteBufferSize = 128*128;
		arduinoSerial.ReadBufferSize = 4096*128;
	...
	public void WriteDegsToSerial () {
		if (arduinoSerial.IsOpen) {
			...
			string [] strs_out = new string [4];

            // Convert all integers to 3-digit string
            strs_out[0] = degs[0].ToString("000");
            strs_out[1] = degs[1].ToString("000");
            strs_out[2] = degs[2].ToString("000");
            strs_out[3] = degs[3].ToString("000");

            // The string to write to Serial
            string str = strs_out[0];
